...
def generate_trained_vectors() -> str:
    """tb_trained_mnist.sv case: a trained network on real MNIST digits.

    Every other block in this file drives random_deploy_parameters, which the
    golden model itself documents as not trained and not expected to classify
    anything. That is deliberate and it is better stimulus -- random weights
    push accumulators into ranges a trained network never reaches. It just
    cannot answer whether the accelerator recognises a digit.

    Both inputs here are checked-in artifacts, so this stays deterministic and
    needs neither mnist.npz nor a retrain in CI:
      golden/trained_params.npz  weights, biases and calibrated shifts
      golden/mnist_demo.npz      the first ten MNIST test digits, in order

    The expected class per image is computed here from deploy_forward_int8
    rather than stored, so there is exactly one source of truth; if the trained
    artifact is ever replaced, these vectors follow it automatically.
    """

    from golden.trained import (
        pad_to_32,
        quantize_image,
        trained_metadata,
        trained_parameters,
    ...
    shifts = trained_shifts()
    meta = trained_metadata()

    demo_path = ROOT / "golden" / "mnist_demo.npz"
    with np.load(demo_path) as archive:
        raw_images = archive["images"]
        labels = archive["labels"].astype(int)

    images = quantize_image(pad_to_32(raw_images))

    predictions = []
    for image in images:
        result = deploy_forward_int8(image, params, shifts=shifts)
        predictions.append(int(result["prediction"]))
    predictions_arr = np.array(predictions, dtype=np.int64)
    label_matches = int((predictions_arr == labels).sum())

    trained_dir = VECTOR_DIR / "trained"
    write_hex(trained_dir / "images.hex", images, 8)
    write_hex(trained_dir / "expected.hex", predictions_arr, 8)
    write_hex(trained_dir / "labels.hex", labels, 8)
    write_hex(trained_dir / "c1_wgt.hex", params["c1_w"], 8)
    write_hex(trained_dir / "c1_bias.hex", params["c1_b"], 32)
    write_hex(trained_dir / "c3_wgt.hex", params["c3_w"], 8)
    write_hex(trained_dir / "c3_bias.hex", params["c3_b"], 32)
    write_hex(trained_dir / "c5_wgt.hex", params["c5_w"], 8)
    write_hex(trained_dir / "c5_bias.hex", params["c5_b"], 32)
    write_hex(trained_dir / "f6_wgt.hex", params["f6_w"], 8)
    write_hex(trained_dir / "f6_bias.hex", params["f6_b"], 32)
    write_hex(trained_dir / "cls_wgt.hex", params["cls_w"], 8)
    write_hex(trained_dir / "cls_bias.hex", params["cls_b"], 32)

    print(
        f"trained_mnist: {len(images)} digits, predicted {list(predictions)}, "
        f"labels {list(labels)}, {label_matches}/{len(images)} correct "
        f"(int8 test-set accuracy {meta['int8_accuracy'] * 100:.2f}%)"
    )

    shift_macros = "\n".join(
        f"`define TV_TR_SHIFT_{name.upper()} {shifts[name]}"
        for name in ("c1", "c3", "c5", "f6")
    )

    return f"""// ---- tb_trained_mnist.sv ----
`define TV_TR_NUM_IMAGES {len(images)}
`define TV_TR_LABEL_MATCHES {label_matches}
`define TV_TR_WATCHDOG_CYCLES {WATCHDOG_CYCLES}
