...
Please do not add claims the repo cannot support. Most shipped vectors are
deterministic random values that verify arithmetic; those are **not trained
MNIST weights**, and no accuracy claim may rest on them. The one exception is
`vectors/trained/`, generated by `golden/train_lenet5.py` from a genuinely
trained and calibrated network — 99.11% INT8, measured once on the MNIST test
set. Quote that number for that tier only, and never recalibrate the per-layer
shifts against test data, which would turn a measured figure into a tuned one.
Likewise,
